...
PYTHON_FILES=(
  ...
  "${PROJECT_ROOT}/CineCMRQC/Testing/Python/CineCMRQCResidentSeriesGuiSmokeTest.py"
...
if command -v pyenv >/dev/null 2>&1 && pyenv versions --bare | grep -qx "3.8.18"; then
  print "[1/9] Python 3.8.18 syntax compatibility"
  PYENV_VERSION=3.8.18 pyenv exec python -m py_compile "${PYTHON_FILES[@]}"
else
  print -u2 "[1/9] SKIPPED: Python 3.8.18 is not installed in pyenv."
fi

if command -v pyenv >/dev/null 2>&1 && pyenv versions --bare | grep -qx "3.12.2"; then
  print "[2/9] Python 3.12.2 syntax compatibility"
  PYENV_VERSION=3.12.2 pyenv exec python -m py_compile "${PYTHON_FILES[@]}"
else
  print -u2 "[2/9] SKIPPED: Python 3.12.2 is not installed in pyenv."
fi

print "[3/9] Slicer module unit and scene round-trip tests"
"${SLICER_EXECUTABLE}" \
  --no-main-window \
...
print "[4/9] Synthetic GUI cardiac phases and mandatory-save switching"
"${SLICER_EXECUTABLE}" \
  --additional-module-paths "${PROJECT_ROOT}/CineCMRQC" \
  --python-script "${PROJECT_ROOT}/CineCMRQC/Testing/Python/CineCMRQCCardiacPhaseGuiSmokeTest.py"

print "[5/9] On-demand loading, single resident series, and incremental frame state"
"${SLICER_EXECUTABLE}" \
  --additional-module-paths "${PROJECT_ROOT}/CineCMRQC" \
  --python-script "${PROJECT_ROOT}/CineCMRQC/Testing/Python/CineCMRQCResidentSeriesGuiSmokeTest.py"

if [[ -z "${PATIENT_PATH}" || ! -d "${PATIENT_PATH}" ]]; then
  print -u2 "[6/9] SKIPPED: set CINE_PATIENT_PATH to an available test-patient folder."
  print -u2 "[7/9] SKIPPED: patient batch test requires the real patient folder."
  print -u2 "[8/9] SKIPPED: overwrite-save test requires the real patient folder."
  print -u2 "[9/9] SKIPPED: GUI real-data test requires the real patient folder."
...
print "[6/9] Real series orientation, ED/ES, edit isolation, labels, and export"
...
print "[7/9] Real patient 14-series / 350-frame batch export"
...
print "[8/9] In-place overwrite, ED/ES metadata, backup, exact filenames, and 4D summary"
CINE_CMR_QC_IMAGE_PATH="${IMAGE_PATH}" \
CINE_CMR_QC_MASK_PATH="${MASK_PATH}" \
CINE_CMR_QC_FLIP_LR=auto \
"${SLICER_EXECUTABLE}" \
...
print "[9/9] Real GUI geometry, cardiac phases, low-confidence fallback, and save guard"
CINE_PATIENT_PATH="${PATIENT_PATH}" \
