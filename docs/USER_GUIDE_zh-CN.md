...
对于当前项目约定的患者目录，使用插件最上方的 `1. 选择患者与动态 Series`：
...
简洁模式下按钮会显示当前 Series，例如 `保存当前 Series：series0015-Body（覆盖原 Mask 并备份）`。该按钮只保存名字中显示的当前 Series，不会顺带保存其他仍在内存中的 Series。正常审核流程不需要重新寻找输出目录：
...
插件先把全部新结果导出到同一磁盘的临时目录；确认 25 帧和 4D 结果完整后，才替换原文件。替换前会在原 `sequence/` 下创建时间戳备份目录，例如 `backup_20260716-213000/`。如果替换过程中发生错误，插件会用备份恢复已经替换的文件，并显示失败信息。
...
segmentation/series0015-Body/
  ...
  sequence/
    ...
    labels.csv
    manifest.csv
    backup_20260716-213000/         # 覆盖前的逐帧文件及旧记录
```
...
- `backup_mask_path`：覆盖保存时，该帧覆盖前文件的备份路径；另存导出时没有此列。
