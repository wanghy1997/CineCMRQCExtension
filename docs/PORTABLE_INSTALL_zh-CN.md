...
将 `CineCMRQCExtension-0.2.1.zip` 解压到固定目录。安装后不要移动或删除该目录。

正确结构：

```text
CineCMRQCExtension-0.2.1/
  CMakeLists.txt
  RELEASE.json
  NOTICE.txt
  CineCMRQC/
...
5. 选择解压后的扩展根目录 `CineCMRQCExtension-0.2.1`，即包含根 `CMakeLists.txt` 的目录。不要只选里面的 `CineCMRQC` 子目录。
6. 出现“添加模块路径”询问时选择确认。
7. 重启 Slicer。
...
3. 选择解压目录中的 `CineCMRQC` 子目录：
   ...
   ```text
   <解压目录>/CineCMRQCExtension-0.2.1/CineCMRQC
   ```
